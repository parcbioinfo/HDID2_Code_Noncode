...
compl_neg$strand <- "-"
```
### Generate the file that will be used to obtain the coverage for the gaps (intronic and intergenic)
```{r eval=FALSE}
gaps_bed <- rbind(compl_pos,compl_neg)
#There is an issue where thereis a 1 base overlap with other (non-gap) features at the beginnning of the start location and at the end of the end location; the base is being removed below
gaps_bed.start<- gaps_bed[,2]+1
gaps_bed.end<-gaps_bed[,3]-1
gaps_bed_interm <- cbind(gaps_bed[,1],as.data.frame(gaps_bed.start[]),as.data.frame(gaps_bed.end[]),gaps_bed[,4:6])
#We need to remove those intervals where the start location is greater than the end location, i.e., there is technically no gap (byproduct of the operation above)
gaps_bed_interm$width<-gaps_bed_interm$gaps_bed.end - gaps_bed_interm$gaps_bed.start
gaps_bed_final<-gaps_bed_interm[gaps_bed_interm$width > 0,1:6]
write.table(gaps_bed_final,"data/mm10_gaps.bed", sep="\t",quote=F,col.names=F,row.names=F)
```
...
### Merge both coverage files (combined code/noncode and gaps), create unique row names (gene_chrom_start), sort and add header to resulting file

```{r eval=FALSE}
genes<-read.table("data/RNASeq016_mm10_code-noncode_coverage_splitoption_stranded.txt",sep="\t")

gaps<-read.table("data/RNASeq016_mm10_gaps_coverage_splitoption_stranded.txt",sep="\t")

genes$unique_id<-paste(genes$V1,genes$V2,genes$V3,genes$V4,sep="_") # ids for exons

gaps$unique_id<-paste(gaps$V1,gaps$V2,gaps$V3,gaps$V4,sep="_")

reads_genes_and_gaps<-rbind(genes,gaps)

reads_genes_and_gaps<-order()
```
